...
void ChunkManager::loadChunkInfo(const ChunkInfoStruct& chunkInfo)
{
...
void ChunkManager::loadListOfAllChunks(const std::vector<ChunkInfoStruct>& chunks)
{
    std::unique_lock lock(mutex_);
...
ChunkInfoStruct ChunkManager::getChunkBySocket(const std::shared_ptr<boost::asio::ip::tcp::socket>& socket) const
{
    std::shared_lock lock(mutex_);
...
void ChunkManager::removeChunkServerDataBySocket(const std::shared_ptr<boost::asio::ip::tcp::socket>& socket)
{
    std::unique_lock lock(mutex_);
    auto it = chunkIdBySocket_.find(socket);
    if (it != chunkIdBySocket_.end()) {
...
void ChunkManager::removeChunkServerDataById(int chunkId)
{
    std::unique_lock lock(mutex_);
    auto it = chunksById_.find(chunkId);
